...
test.describe('revised preprint', () => {
  ...
  test.afterEach(async () => {
    ...
    const versions = (name === 'revised-multiple') ? 3 : 2;
  ...
  test('multiple versions of revised preprint are available', async ({ page }) => {
    ...
    await expect(page.locator('.descriptors__identifier')).toContainText('https://doi.org/10.7554/eLife.revised-multiple-msid.1');
    ...
    const reviewTimelinePageLocatorV1 = page.locator('.review-timeline__list>.review-timeline__event:nth-child(5)');
    ...
    await expect(page.locator('.descriptors__identifier')).toContainText('https://doi.org/10.7554/eLife.revised-multiple-msid.2');

    // 3rd child is 2nd description details (<dd>) from the timeline definition list
    const reviewTimelinePageLocatorV2 = page.locator('.review-timeline__list>.review-timeline__event:nth-child(3)');
    ...
    await expect(page.locator('h1.title')).toHaveText('OpenApePose: a database of annotated ape photographs for pose estimation (revised)');
    await expect(page.locator('.article-status__text')).toContainText('Revised by authors after peer review.');
    await expect(page.locator('.descriptors__identifier')).toContainText('https://doi.org/10.7554/eLife.revised-multiple-msid.3');

    // 1st child is 1st description details (<dd>) from the timeline definition list
    const reviewTimelinePageLocatorV3 = page.locator('.review-timeline__list>.review-timeline__event:nth-child(1)');
    await expect(reviewTimelinePageLocatorV3).toContainText('Reviewed preprint version 3');
